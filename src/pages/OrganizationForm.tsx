  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="p-2 rounded-md bg-muted hover:bg-border text-muted-foreground hover:text-foreground transition-colors">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="card space-y-4">
          <h2 className="text-sm font-semibold text-foreground border-b border-border pb-3 uppercase tracking-wide">Dados principais</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-muted-foreground mb-1">Nome da entidade *</label>
              <input required disabled={isReadOnly} type="text" className="input disabled:opacity-60" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Website</label>
              <input type="url" disabled={isReadOnly} className="input disabled:opacity-60" value={formData.site} onChange={e => setFormData({ ...formData, site: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">País</label>
              <input type="text" disabled={isReadOnly} className="input disabled:opacity-60" value={formData.country} onChange={e => setFormData({ ...formData, country: e.target.value })} />
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-muted-foreground mb-1">Descrição curta</label>
              <textarea disabled={isReadOnly} className="input h-24 resize-none disabled:opacity-60" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
            </div>
          </div>
        </div>
        ...
        {/* Contato Principal */}
        <div className="card space-y-4">
          <h2 className="text-sm font-semibold text-foreground border-b border-border pb-3 uppercase tracking-wide flex items-center gap-2">
            <User className="w-4 h-4 text-accent" />
            Contato principal
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Nome do contato</label>
              <input
                type="text"
                disabled={isReadOnly}
        ...
        <div className="card space-y-4">
          <div className="flex items-center justify-between border-b border-border pb-3">
            <div>
              <h2 className="text-sm font-semibold text-foreground uppercase tracking-wide">Classificação (smart tags)</h2>
              <p className="text-[11px] text-muted-foreground mt-0.5">O motor de busca usará essas tags.</p>
            </div>
          </div>
            ...
            {!isReadOnly && (
              <div>
                ...
                {availableTags.length > 0 ? (
                  ...
                  <p className="text-xs text-muted-foreground italic">Nenhuma tag encontrada. Crie novas tags no menu correspondente.</p>
                )}
              </div>
            )}

            <div className="p-4 rounded-md bg-muted border border-border min-h-[100px]">
              <p className="text-xs font-medium text-muted-foreground mb-3">Tags selecionadas:</p>
              <div className="flex flex-wrap gap-2">
                {formData.tags?.length === 0 && <span className="text-sm text-muted-foreground/70">Nenhuma tag selecionada.</span>}
