...
import { Save, Trash2, ArrowLeft, Building2 } from 'lucide-react';
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">
            {isEditing ? 'Atualize os dados e as classificações.' : 'Cadastre uma nova entidade e classifique-a.'}
          </p>
        </div>
      </div>
        ...
        <div className="card space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input required type="text" className="input-dark" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">Website</label>
              <input type="url" className="input-dark" value={formData.site} onChange={e => setFormData({ ...formData, site: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">País</label>
              <input type="text" className="input-dark" value={formData.country} onChange={e => setFormData({ ...formData, country: e.target.value })} />
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-slate-400 mb-1">Descrição Curta</label>
              <textarea className="input-dark h-24 resize-none" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
            </div>
          </div>
        </div>
